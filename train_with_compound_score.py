...
model2.compile(optimizer=keras.optimizers.Adam(learning_rate), loss='mse')

# Define the target network model
target_model2 = keras.models.clone_model(model2)
...
def experience_replay(agent_memory):
    ...
    current_Q = model2.predict(states)
    next_Q = target_model2.predict(next_states)
    ...
    model2.fit(states, current_Q, epochs=1, verbose=0)
...
for episode in range(num_episodes):
    ...
    for i in range(len(data)):
        ...
        state = np.array([data['Open'][i], data['5-day MA'][i], portfolio_value, cash_held, stocks_held, data['compound'][i-1]])
        ...
        if np.random.rand() < exploration_rate:
            ...
            Q_values = model2.predict(np.array([state]))
        ...
        next_state = np.array([data['Open'][i], data['5-day MA'][i], portfolio_value, cash_held, stocks_held, data['compound'][i-1]])
...
model2.save('/mnt/home/mahaseth/CMSEProj/model2.h5')
