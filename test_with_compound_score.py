...
model2 = load_model('/mnt/home/mahaseth/CMSEProj/model2.h5')
...
for i in range(len(test_data)):
    ...
    state = np.array([test_data['Open'][i], test_data['5-day MA'][i], portfolio_value, cash_held, stocks_held, test_data['compound'][i-1]])
...
portfolio_values2 = portfolio_values
# Plot the portfolio value over time
plt.plot(range(len(test_data)), portfolio_values)
plt.xlabel('Day')
plt.ylabel('Portfolio Value')
plt.savefig('portfolio_values_2.png')
plt.show()
